...
open class VersaPlayerView: View, PIPProtocol {
    ...
    open func layout(view: View, into: View? = nil, isFullScreen: Bool = true) {
...
//            view.bottomAnchor.constraint(equalTo: into.bottomAnchor).isActive = true
            ...
            view.widthAnchor.constraint(equalTo: into.widthAnchor, multiplier: 0.80).isActive = true
            view.heightAnchor.constraint(equalTo: into.heightAnchor, multiplier: 0.60).isActive = true
            
            view.centerXAnchor.constraint(equalTo: into.centerXAnchor, constant: 0).isActive = true
            view.centerYAnchor.constraint(equalTo: into.centerYAnchor, constant: 32).isActive = true
        }
    ...
    open func setFullscreen(enabled: Bool) {
        ...
        if enabled {
            ...
            if let window = UIApplication.shared.keyWindow {
                nonFullscreenContainer = superview
                removeFromSuperview()
                layout(view: self, into: window, isFullScreen: true)
            }
            #endif
        }else {
            removeFromSuperview()
            layout(view: self, into: nonFullscreenContainer, isFullScreen: false)
        }
